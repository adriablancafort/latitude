# Deployment

Deployment is single-branch by default. `development` is the trunk branch and normal merges to it deploy the merged commit to staging.

Production deployments are release-tag based. A developer deploys a commit to production by pushing a `vX.Y.Z` tag that points at a commit already reachable from `origin/development`. Use `scripts/release.sh [version]` from the commit to tag and push it. If no version is supplied, the script finds the latest `vX.Y.Z` tag and bumps to the next patch version, for example `v1.4.2` → `v1.4.3`; use `--minor` or `--major` to bump those components instead. If no release tag exists, it starts at `v0.1.0`.

The production workflow validates that the tag is a `vX.Y.Z` tag, that the tagged commit is reachable from `origin/development`, and that the tagged commit already has a successful staging deployment.

Public package publishing is decoupled from production deployment. Package publish workflows run on every merge to `development`; each package no-ops unless its package version differs from the registry. When a package version changes, publishing requires the package's own changelog entry for that version. Package versions and package changelogs remain developer-managed and are not generated by the monorepo release script.

Do not use `main` as a production promotion branch.


## Infrastructure network diagram

The current production network diagram lives in [`./network-diagram.md`](./network-diagram.md), with the rendered SVG at [`./assets/latitude-network-diagram.svg`](./assets/latitude-network-diagram.svg). Use this diagram for security reviews and compliance evidence that asks for the hosted network diagram.
